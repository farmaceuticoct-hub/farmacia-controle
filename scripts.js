 ...
 * Sistema: Assistência Farmacêutica - Controlados e Especiais (sem Diabetes)
 * Correções: Sincronização forçada, bloqueio de botão, backup manual, leitura estendida.
 */
...
let mesCtrl = 'todos', mesEsp = 'todos';
let dashboardMesCtrl = null, dashboardMesEsp = null;
...
function inicializar() {
    ...
    console.log(`🚀 v${VERSAO} - Controlados + Especiais (sem Diabetes)`);
}
...
async function sincronizarTudo(force = false) {
    ...
    try {
        ...
        const dp = await lerPlanilha(SHEET_ID_PACIENTES, 'Pacientes!A1:F5000');
        if (dp.length > 1) {
        ...
        // 2. CONTROLADOS (RANGE ESTENDIDO)
        console.log('📥 Lendo controlados...');
        const dc = await lerPlanilha(SHEET_ID_CTRL, 'Registros!A1:G5000'); // ← aumentado para 5000 linhas
        if (dc.length > 1) {
        ...
        try {
            const de = await lerPlanilha(SHEET_ID_ESP, 'Página1!A1:H5000');
            if (de.length > 1) {
...
function carregarSelectPacientesCtrl() {
    const listas = ['listaPacientesCtrl', 'listaPacientesEsp'];
...
async function registrarControlado() {
    if (salvandoControlado) {
        mostrarStatus('⏳ Aguarde, registrando...', 'info');
        return;
    }
    salvandoControlado = true;
    const btn = document.querySelector('#modulo-controlados .btn-primary');
    ...
    try {
        ...
        if (q <= 0) {
            mostrarStatus('⚠️ Quantidade deve ser maior que zero!', 'alerta');
            return;
        }
        let rep = 'Não';
...
async function registrarEspecial() {
    ...
    salvandoEspecial = true;
    const btn = document.querySelector('#modulo-especiais .btn-roxo');
    if (btn) { btn.disabled = true; btn.textContent = '💉 Salvando...'; }
    ...
    try {
        ...
        if (a > ea) {
            mostrarStatus(`⚠️ Estoque insuficiente!\nDisponível: ${ea} | Solicitado: ${a}`, 'alerta');
            return;
        }
        estoque[m] = ea - a;
        const ne = estoque[m];
...
function trocarConfigTab(tab) {
    ...
    const tabs = ['medicamentos', 'limites', 'backup', 'sobre', 'estoque'];
